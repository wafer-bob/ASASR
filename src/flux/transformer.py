...
def tranformer_forward(
    ...
    **params: dict,
    ...
    if guidance is not None:
        ...
        # FLUX.1-dev is a guidance-distilled model, so time_text_embed must receive a guidance value.
        # During training no guidance is passed, so it defaults to 0 here (matching the original
        # training setup); during inference generate() passes guidance_scale (e.g. 3.5), so this
        # branch is not triggered.
        guidance = torch.zeros(
            timestep.shape[0], device=timestep.device, dtype=hidden_states.dtype
        )

    temb = self.time_text_embed(timestep, guidance, pooled_projections)
    cond_temb = self.time_text_embed(
